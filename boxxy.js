...
(() => {
  ...
  const PACK_ARTWORK = Object.freeze({
    ...
    "starry-night": {
      desktop: "assets/pack-art/starry-night-banner.webp",
      mobile: "assets/pack-art/starry-night-mobile.webp",
      alt: "Boxxy doing a star jump in a sea of glowing stars"
    }
  });
  ...
  function captureBoxxyAnalytics(eventName, properties = {}) {
    if (makerDailyPractice || window.BOXXY_PRIVATE_PRACTICE) return;
    try {
      const payload = {
        game: "BOXXY",
  ...
  function packRewardKind(pack) {
    ...
    if (pack?.id === STARRY_NIGHT_PACK_ID) return "moon";
    return "star";
  }
  ...
  function packRewardSvg(pack) {
    ...
    if (kind === "alphabet") return PACK_ALPHABET_SOUP_BADGE_HTML;
    if (kind === "moon") return PACK_STARRY_NIGHT_BADGE_HTML;
  ...
  function buildDailyArchiveCard(puzzle, result, { featured = false } = {}) {
    ...
    if (result) {
      ...
      movesStat.innerHTML = `<span>BEST MOVES</span><strong>${Math.max(0, Number(result.moves) || 0)}</strong>`;
  ...
  function scheduleDailyMidnightRefresh() {
    if (window.BOXXY_PRIVATE_PRACTICE) return;
  ...
  const KONAMI_SHOWCASE_MUSIC = "assets/audio/Tetris-Piano-293KB.mp3";
  ...
  function preloadKonamiShowcaseColourAssets() {
    const colours = [...new Set(goals.map(goal => GOAL_COLOURS?.normalise?.(goal.colour) || "red"))];
    return Promise.all(colours.flatMap(colour => [
      ensureBoardAsset(boardAssetPath("goal", colour)),
      ensureBoardAsset(boardAssetPath("box", colour))
    ])).catch(() => []);
  }
  ...
  function runKonamiShowcaseSolution() {
    ...
    preloadKonamiShowcaseColourAssets().finally(() => {
      if (!autoplayRunning || !konamiShowcaseActive) return;
      refreshBoardArtwork();
  ...
  if (window.BOXXY_PRIVATE_PRACTICE) {
    const puzzle = window.BOXXY_PRIVATE_PRACTICE.puzzle;
    const result = puzzle && loadMakerTest(puzzle.layout, "", {
      name: `Daily #${puzzle.sequence} · ${puzzle.date}`,
      goalColours: puzzle.goalColours || {}, rainbowMode: Boolean(puzzle.rainbowMode),
      practiceDaily: { date:puzzle.date, sequence:puzzle.sequence, name:puzzle.name }
    });
    if (!result?.ok && thoughtText) thoughtText.textContent = result?.error || "Practice could not be loaded.";
  } else if (SHARED_PUZZLE_PAYLOAD?.ok) {
  ...
  completeStartupSplash().then(() => {
    if (!window.BOXXY_PRIVATE_PRACTICE) window.setTimeout(() => showDailyInvite(false), 180);
  }).catch(error => {
